...
class AnalysisJobPostingEventHandlerTest {
  ...
  @Test
  void shouldDelegateAnalysisToTheCorePort() {
    final var handler = new AnalysisJobPostingEventHandler(processJobPostingCreatedPort);

    assertThat(handler.type()).isEqualTo(JobPostingEventType.ANALYSIS);
    handler.handle(event());

    verify(processJobPostingCreatedPort, description("analysis delegated to the core"))
      .analyzePosting(any());
  }
